            ...
            <div data-kt-stepper-element="content">
              <div class="w-100" v-if="isCreatingQuote === false">
                <div class="pb-8 pb-lg-10">
                  ...
                  <h2 v-else class="fw-bold text-danger">Something Went Wrong, Plase Try Again Later!</h2>
                </div>
              </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const createQuote = async () => {
      ...
      let request = await axios.post('http://127.0.0.1:8000/api/leads/create/', {
        car_make: formData.value.carMake?.value,
        car_model: formData.value.carModel?.value,
        car_year: formData.value.carYear?.value,
